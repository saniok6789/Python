...
RES = WIDTH, HEIGHT = 1202, 902 #это инециализация окна
TILE = 30 #это инециализация окна
cols, rows = WIDTH // TILE, HEIGHT // TILE #это инециализация окна

pygame.init()
sc = pygame.display.set_mode(RES)
clock = pygame.time.Clock()

class Cell: #нужно для характеристики клеток (яцеек)
    def __init__(self, x, y):
        self.x, self.y = x, y
        self.walls = {'top': True, 'right': True, 'bottom': True, 'left': True}
        self.visited = False
    def draw_corrent_cell(self):
        x,y=self.x*TILE,self.y*TILE
        pygame.draw.rect(sc,pygame.Color('saddlebrown'),(x+2,y+2,TILE-2,TILE-2))
    def draw(self): #для рисовки клеток (яцеек)
        x, y = self.x * TILE, self.y * TILE
        if self.visited:
...
while True:
    ...
    current_cell.draw_corrent_cell()
